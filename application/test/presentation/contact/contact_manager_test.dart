...
void main() {
  ...
  ContactScreenManager create() => ContactScreenManager(
        ...
        bugReportingService,
        extractLogUseCase,
        MockContactNavActions(),
        shareUriUseCase,
        featureManager,
      );
  blocTest<ContactScreenManager, ContactScreenState>(
  ...
  test(
    ...
    () {
      ...
      expect(
        () => manager.openExternalUrl(url, CurrentView.contact),
        returnsNormally,
      );
  ...
  blocTest<ContactScreenManager, ContactScreenState>(
    ...
    build: () => create(),
    act: (manager) =>
        manager.openExternalUrl('https://xayn.com', CurrentView.contact),
    //default one, emitted when manager created
    expect: () => [stateReady],
    verify: (manager) {
      verifyInOrder([
