...
@lazySingleton
class HandleSourceSelectionShownUseCase extends UseCase<None, None> {
  final AppStatusRepository _repository;
  bool hasBeenShown = false;

  HandleSourceSelectionShownUseCase(this._repository);
  ...
  Stream<None> transaction(param) async* {
    ...
    if (!hasBeenShown) {
      ...
      final sourceSelection = appStatus.cta.sourceSelection;

      /// Update the status of the data
      final updatedSourceSelection = sourceSelection.copyWith(
          numberOfTimesShown: sourceSelection.numberOfTimesShown + 1);
      final updatedCta = cta.copyWith(sourceSelection: updatedSourceSelection);
