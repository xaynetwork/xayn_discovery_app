...
class HostedDocumentMapper extends Mapper<Map<String, dynamic>, Document> {
  ...
  static final _formatterA = DateFormat('yyyy-MM-dd hh:mm:ss');
  static final _formatterB = DateFormat('yyyy-MM-dd');

  @override
  Document map(Map<String, dynamic> input) => Document(
        documentId: DocumentId.fromString(
            input[HostedDocumentMapperFields.id] as String),
        stackId: StackId.nil(),
  ...
  DateTime _parseDateTime(String? input) {
    ...
    try {
      date = _formatterA.parse(input);
    } catch (_) {
      date = _formatterB.parse(input);
    }
