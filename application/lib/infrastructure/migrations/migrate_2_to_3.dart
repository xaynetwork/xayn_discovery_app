...
class Migration_2_To_3 extends BaseDbMigration {
  ...
  Future<int> runMigration(int fromVersion) async {
    ...
    // Check if the user has any feed data stored in the database.
    // If so, we assume it was a beta user.
    final recordBox = HiveCrdt(Hive.box<Record>('feed'), HiveDB.nodeId);
    final isBetaUser = !recordBox.isEmpty;
    final repository = SingleValueMigrationRepository(
      ...
      transform: (map) {
        map[AppStatusFields.isBetaUser] = isBetaUser;
        return map;
      },
    );
    repository.migrate();
